...
const IdeaCard = ({ idea, onLikeToggle }) => {
  ...
  useEffect(() => {
    ...
    checkIfLiked();
    fetchUserProfile();
  }, [user, idea.id, idea.user_id, supabase]);
  ...
  return (
    <div className="idea-card bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6 transition-colors duration-300">
      ...
      <div className="flex justify-between items-center">
        ...
        <button 
          onClick={handleLike}
          className="flex items-center space-x-1 text-gray-600 dark:text-gray-400 hover:text-telkom-red transition-colors"
        >
          <svg 
            className={`w-5 h-5 ${isLiked ? 'fill-telkom-red' : ''}`} 
            viewBox="0 0 24 24"
          >
